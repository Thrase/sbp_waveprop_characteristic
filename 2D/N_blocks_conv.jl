...
F(x, y, t, e) = begin #u_rr + (1/r)*u_r + (1/r^2)*u_theta,theta
    ...
    if EToDomain[e] == 1
        ...
        u_rr =
            c .* exp.(-1 .* r .^ 2) .* (6 .* r .- 4 .* r .^ 3) .* sin.(theta)
        u_thetatheta = -c .* (1 .- exp.(-1 .* r .^ 2)) .* r .* sin.(theta)
...
for lvl in 1:length(ϵ)
    ...
    #ddddd
    OPTYPE =
        typeof(rhsoperators(rho, p, 12, 12, mets, gDfun, gDdotfun, gNfun, F))
    ...
    for e in 1:nelems
        ...
        qexact_v =
            ue_t.(metrics[e].coord[1][:], metrics[e].coord[2][:], tspan[2], e)
        qexact_û1 =
            ue.(
